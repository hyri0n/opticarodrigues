  ...
  <div class="w3-main" style="margin-top:3.5%;margin-left:300px">
    <div class="w3-container w3-round">
      ...
      ?>
            ...
            <form class="w3-container" action="acoes_mensalidade.php?cad=true" method="post">
              <div class="w3-section">
                ...
                <select class="w3-select w3-border-grey w3-round-small" name="oscliente">
                    <option value="" disabled selected>Os - Cliente</option>
                    <?php
                    include_once "conexao.php";
                    $stmt = $conn-> query ('SELECT os,nome FROM clientes');
                    while ($linhashow = $stmt-> fetch(PDO::FETCH_ASSOC)){
                    ?>
                    <?php echo"<option value='".$linhashow['os']."'>"; ?><?php echo $linhashow['os']." - ".$linhashow['nome']; echo "</option>";}; ?>
                </select>
        ...
        <?php
        include_once "conexao.php";
        $stmt = $conn-> query ('SELECT mensalidades.*, clientes.nome FROM mensalidades INNER JOIN clientes ON os = mensalidades.os_cliente ORDER BY mensalidades.validade DESC;');
        ?>
        ...
        <div class="w3-container">
          <header class="w3-container w3-teal w3-round-small">
           ...
           <span><input type="text" id="myInputB" onkeyup="myFunctionShow()" placeholder="Buscar Por.."></span>
